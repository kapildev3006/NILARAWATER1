...
export default function AssignRouteDriverModal({ isOpen, onClose, route, onAssigned }) {
  const [drivers, setDrivers] = useState([]);
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [selectedDriverId, setSelectedDriverId] = useState(route?.deliveryPartner?._id || route?.deliveryPartner || null);
  ...
  useEffect(() => {
    if (isOpen && route) {
      loadDrivers();
      setSelectedDriverId(route.deliveryPartner?._id || route.deliveryPartner || null);
      setErrorMsg("");
  ...
  const handleSave = async () => {
    ...
    try {
      setSubmitting(true);
      setErrorMsg("");
      const res = await fetchWithAuth(`/admin/delivery-routes/${route._id}/assign-driver`, {
        method: "POST",
      ...
      if (res.success) {
        ...
        setErrorMsg(res.message || "Failed to update route driver assignment");
      }
    } catch (err) {
      console.error("Error updating route driver assignment:", err);
      setErrorMsg("Failed to assign driver to route.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h2 className="text-lg font-black text-slate-800">Assign Driver to Batch Route</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Route #{route.routeNumber} • {route.area} ({route.totalStops} Stops)
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-2">
          ...
          >
            <div>
              <p className="text-xs font-bold text-slate-700">No Driver Assigned</p>
              <p className="text-[11px] text-slate-400">Leave route unassigned for later dispatch</p>
            </div>
            {selectedDriverId === null && (
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center gap-3">
          ...
          >
            ...
            {submitting ? "Assigning..." : "Assign Entire Route"}
          </button>
        </div>
